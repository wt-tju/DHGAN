...
class GAT(nn.Module):
    def __init__(self, node_nums, nodeid_emb_dim, sparse_maxs, sparse_emb_dim, hidden_dim, output_dim, n_layers, negs_nums, dropout, etypes, use_nodeid_emb):
        ...
        for mode in etypes:
            self.layers[mode].append(GATLayer(dropout=dropout, hidden_dim=hidden_dim, mode=mode))
...
class GATLayer(nn.Module):
    def __init__(self, dropout, hidden_dim, mode):
        super(GATLayer, self).__init__()
        self.attn_l = nn.Parameter(torch.zeros(size=(1, hidden_dim)))
        self.attn_r = nn.Parameter(torch.zeros(size=(1, hidden_dim)))
        self.mode = mode
